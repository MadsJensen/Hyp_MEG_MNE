...
for h in range(len(bands)):
    ...
    [stc[band.keys()[0]].crop(0, 0.2) for stc in stcs_normal]
    [stc[band.keys()[0]].crop(0, 0.2) for stc in stcs_hyp]
    ...
    for label in labels:
        ...
        outfile_p_name = "p_results_BA_tone_power" +\
            "_%s_MNE_0-02_%s_nostd_mean_flip.csv" % (band.keys()[0],
                                                     clf_names[0])
        outfile_score_name = "score_results_BA_tone_power" +\
            "_%s_MNE_0-02_%s_nostd_mean_flip.csv" % (band.keys()[0],
                                                     clf_names[0])
