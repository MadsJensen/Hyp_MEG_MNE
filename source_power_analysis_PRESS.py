...
for band in bands:
    ...
    print "load normal"
    stcs_normal =\
        pickle.load(open("stcs_normal_press_source_induced_%s_0-05_DKT.p"
                         % band, "rb"))

    print "load hyp"
    stcs_hyp =\
        pickle.load(open("stcs_hyp_press_source_induced_%s_0-05_DKT.p" % band,
                         "rb"))
    ...
    n_labels_normal = len(labels)  # labelTsNormalCrop.shape[1]

    n_trials_hyp = 74  # labelTsHypCrop.shape[0]
    n_labels_hyp = len(labels)  # labelTsHypCrop.shape[1]
    ...
    for degreeNumber in range(binMatrixHyp.shape[0]):
        ...
        pickle.dump(pvalList,
                    open("power_press_MI_DKT_%s_0-05_deg.p" % band, "wb"))
    # %% for CC
    ...
    for ccNumber in range(binMatrixHyp.shape[0]):
        ...
        pickle.dump(pvalListCC,
                    open("power_press_MI_DKT_%s_0-05_CC.p" % band, "wb"))
