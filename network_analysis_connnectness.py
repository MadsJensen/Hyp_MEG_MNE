...
for band in bands.keys():
    ...
    # eff_normal = np.empty(cohMatrixNormal.shape[2])
    # for j in range(cohMatrixNormal.shape[2]):
    #     tmp = cohMatrixNormal[:, :, j]
    #     eff_normal[j] = bct.efficiency_wei(tmp)

    # eff_hyp = np.empty(cohMatrixHyp.shape[2])
    # for j in range(cohMatrixHyp.shape[2]):
    #     tmp = cohMatrixHyp[:, :, j]
    #     eff_hyp[j] = bct.efficiency_wei(tmp)

    pval, obs_diff, diffs =\
        permutation_test(eff_normal, eff_hyp, 10000, np.mean)
    print band, pval, obs_diff, np.mean(diffs)
    print "mean diff", np.mean(eff_normal) - np.mean(eff_hyp)
    pvals += [pval]
